...
The RPM spec file provided here is for Cl-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://clcrypto.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Cl git repository](https://github.com/JustAResearcher/Cl) but are not part of
...
Currently only `Source10` is used, for the example `cl.conf` file.
...
in the cl git repository. At present only an SVG version of the Cl
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cl
...
needed when 0.12.1 is released, a proper fix is already in the Cl git
master branch.
...
idea to build Cl Core against a static version of that library compiled
...
The `cl.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cl-server` package. At this point, most RPM based
...
The `cl-server` package should have SELinux support. How to properly do
...
Cl-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cl/). He has been
packaging Cl for Fedora at least since 2012.
...
4. I renamed the `cl` package that contains the Qt GUI to `cl-core` as
that appears to be how the general population refers to it, in contrast to
`cl-xt` or `cl-classic`. I wanted to make sure the general population
