...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** CL START */
    case OP_cl_ASSET              : return "OP_cl_ASSET";
    /** CL END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_cl_ASSET) { // OP_cl_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == cl_N) { // Check to see if cl starts at 27 ( this->size() < 105)
                if ((*this)[28] == cl_E)
                    if ((*this)[29] == cl_X)
                        index = 30;
            } else {
                if ((*this)[28] == cl_N) // Check to see if cl starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == cl_E)
                        if ((*this)[30] == cl_X)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == cl_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == cl_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == cl_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == cl_N) {
...
{
    // 1 OP_cl_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_cl_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_cl_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_cl_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_cl_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
