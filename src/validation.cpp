...
#if defined(NDEBUG)
# error "Cl cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Cl Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s cl additional fees, %d delta bytes\n",
...
{
    CAmount nSubsidy = 500 * COIN;  // Initial subsidy after the fixed period

    if (nHeight >= 86000 && nHeight < 126000) {
        // First halving: Subsidy is 250 COIN for blocks between 86,000 and 126,000
        nSubsidy = 250 * COIN;
    } else if (nHeight >= 126000 && nHeight < 136000) {
        // Second halving: 25% reduction for blocks between 126,000 and 136,000
        nSubsidy = 187.50 * COIN;
    } else if (nHeight >= 136000) {
        // Subsidy reduces by 5% every 15,000 blocks after block 136,000
        int halvings = (nHeight - 136000) / 15000;
        for (int i = 0; i < halvings; ++i) {
            nSubsidy *= 0.95;  // 5% reduction in subsidy for each halving
        }
        nSubsidy = std::max(nSubsidy, static_cast<CAmount>(0));  // Ensure subsidy is not negative
    } else if (nHeight >= 650000) {
        // Set subsidy to 0 for blocks with height 650,000 and above
        nSubsidy = 0;
    }

    // Default case: Initial subsidy is 500 COIN
    return nSubsidy;
}
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** CL START */ /* Pass assetCache into function */ /** CL END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** CL START */ /* Pass assetCache into function */ /** CL END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** CL START */ /* Pass assetsCache into the SpendCoin function */ /** CL END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** CL START */ /* Pass assetsCache into ApplyTxInUndo function */ /** CL END */
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
    ...
    if (!IsValidDestination(destCommunityAutonomous)) {
		LogPrintf("IsValidDestination: Invalid Cl address %s \n", GetCommunityAutonomousAddress);
    }
	// Parse Cl address
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** CL START */ /** CL END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush cl: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** CL START */ /*Add asset to function */ /** cl END*/
