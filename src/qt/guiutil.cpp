...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a cl address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new ClAddressEntryValidator(parent));
    widget->setCheckValidator(new ClAddressCheckValidator(parent));
}
...
bool parseClURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no cl: URI
    if(!uri.isValid() || uri.scheme() != QString("cl"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ClUnits::parse(ClUnits::cl, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseClURI(QString uri, SendCoinsRecipient *out)
{
    // Convert cl:// to cl:
    //
    //    Cannot handle this later, because cl:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cl://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "cl:");
    }
    QUrl uriInstance(uri);
    return parseClURI(uriInstance, out);
}

QString formatClURI(const SendCoinsRecipient &info)
{
    QString ret = QString("cl:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ClUnits::format(ClUnits::cl, info.amount, false, ClUnits::separatorNever));
        paramCount++;
    }
...
bool openClConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CL_CONF_FILENAME));

    /* Open cl.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Cl.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Cl (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Cl (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "cl.desktop";
    return GetAutostartDir() / strprintf("cl-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a cl.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Cl\n";
        else
            optionFile << strprintf("Name=Cl (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the cl app
...
bool GetStartOnSystemStartup()
{
    CFURLRef clAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (clAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, clAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add cl app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, clAppUrl, nullptr, nullptr);
    }
