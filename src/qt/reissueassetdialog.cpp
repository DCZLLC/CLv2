...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + ClUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid cl Destination Address"));
            return;
        }
    }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " cl";
    amount.append("</b>");
