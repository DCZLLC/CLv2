...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(ClUnits::cl),
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    ui->clBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->clBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(ClUnits::formatWithUnit(unit, balance, false, ClUnits::separatorAlways));
    ui->labelUnconfirmed->setText(ClUnits::formatWithUnit(unit, unconfirmedBalance, false, ClUnits::separatorAlways));
    ui->labelImmature->setText(ClUnits::formatWithUnit(unit, immatureBalance, false, ClUnits::separatorAlways));
    ui->labelTotal->setText(ClUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance, false, ClUnits::separatorAlways));
    ui->labelWatchAvailable->setText(ClUnits::formatWithUnit(unit, watchOnlyBalance, false, ClUnits::separatorAlways));
    ui->labelWatchPending->setText(ClUnits::formatWithUnit(unit, watchUnconfBalance, false, ClUnits::separatorAlways));
    ui->labelWatchImmature->setText(ClUnits::formatWithUnit(unit, watchImmatureBalance, false, ClUnits::separatorAlways));
    ui->labelWatchTotal->setText(ClUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, ClUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("cl")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the cl balance grid from expanding and looking terrible when asset balance is hidden
