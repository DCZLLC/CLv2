...
const std::string ClGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ClGUI::DEFAULT_WALLET = "~Default";

ClGUI::ClGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ClGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a cl address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cl: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to cl addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your cl addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified cl addresses"));
    ...
    openAction->setStatusTip(tr("Open a Cl: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible cl command-line options").arg(tr(PACKAGE_NAME)));
...
void ClGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/clcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* clLabelLayout = new QVBoxLayout(toolbarWidget);
        clLabelLayout->addWidget(label);
        clLabelLayout->addWidget(toolbar);
        clLabelLayout->setDirection(QBoxLayout::TopToBottom);
        clLabelLayout->addStretch(1);
        ...
        // Get the latest Cl release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void ClGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ClGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ClGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ClGUI::setWalletActionsEnabled(bool enabled)
{
...
void ClGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ClGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ClGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void ClGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ClGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void ClGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void ClGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void ClGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ClGUI::openClicked()
{
...
void ClGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ClGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ClGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ClGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ClGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ClGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CL START */
void ClGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void ClGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void ClGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void ClGUI::gotoRestrictedAssetsPage()
{
...
void ClGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to cl network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ClGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ClGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ClGUI::updateHeadersSyncProgressLabel()
{
...
void ClGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ClGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Cl"); // default title
...
void ClGUI::changeEvent(QEvent *e)
{
...
void ClGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ClGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ClGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "cl")
        msg += tr("Amount: %1\n").arg(ClUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ClUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void ClGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ClGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ClGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ClGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ClGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void ClGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ClGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ClGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ClGUI::detectShutdown()
{
...
void ClGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ClGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ClGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ClGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(ClGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void ClGUI::subscribeToCoreSignals()
{
...
void ClGUI::unsubscribeFromCoreSignals()
{
...
void ClGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ClGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ClUnits::Unit> units = ClUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ClUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(ClUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(ClUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (ClUnits::Unit u : ClUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ClUnits::name(u)), this);
...
void ClGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=clBTC"));
...
void ClGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/JustAResearcher/Cl/releases"));
