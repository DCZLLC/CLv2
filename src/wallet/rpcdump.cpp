...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Cl address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The cl address for the private key\n"
            "\nResult:\n"
    ...
    return CClSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to cld)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/JustAResearcher/Cl/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Cl %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CClSecret(key).ToString(), strTime);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                // Get the Cl Ext Key from the master key
                CClExtKey b58extkey;
                b58extkey.SetKey(masterKey);
                ...
                // Get the Cl Ext Key from the public key
                CClExtPubKey b58extpubkey;
                b58extpubkey.SetKey(pubkey);
                ...
                // Create the Cl Account Ext Private Key
                CClExtKey b58accountextprivatekey;
                b58accountextprivatekey.SetKey(accountKey);

                // Create the Cl Account Ext Public Key
                CClExtPubKey b58actextpubkey;
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see cld log for details) and could "
