...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for cl
    ...
    if (p_rewardSnapshot.strDistributionAsset != "cl") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is cl with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using cl differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "cl") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Cl address (already validated during ownership snapshot creation)
