...
find ${DISTNAME}/bin -type f -executable -exec ../Cl/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../Cl/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
rm -rf win64
cp -rf Cl/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/Cl/cl-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf Cl/contrib/windeploy ~/sign/win32
...
mkdir -p ~/Cl/depends/SDKs
cp ~/MacOSX10.11.sdk.tar.gz ~/Cl/depends/SDKs/MacOSX10.11.sdk.tar.gz
cd ~/Cl/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz
