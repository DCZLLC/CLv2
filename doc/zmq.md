...
The CL Core daemon can be configured to act as a trusted "border
router", implementing the cl wire protocol and relay, making
...
The ZeroMQ feature in CL Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cld:
...
For instance:
    ...
    $ cld -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/cld.tx.raw
...
These options can also be provided in cl.conf.
...
From the perspective of cld, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cld directly. Furthermore, no information is
...
using. CLd appends an up-counting sequence number to each
