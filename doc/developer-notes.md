...
The CLcoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play cls" on the test network, if you
...
CL Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- CLMiner : Generates cls (if wallet is enabled).
...
However, in open source software such as CL Core, where everyone uses
...
pay attention to for reviewers of CL Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: CL Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against CLcoin so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cl-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cl-cli` and the GUI debug console use this table to determine how to
